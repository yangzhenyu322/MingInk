package com.mingink.order.utils;
...
public class WxPayV3Util {
    ...
    private PayRefundParams payRefundParams;

    private PayParams payParams;

    public WxPayV3Util(PayRefundParams payRefundParams) {
        this.payRefundParams = payRefundParams;
    }

    public WxPayV3Util(PayParams payParams) {
        this.payParams = payParams;
    }

    public WxPayV3Util(PayParams payParams, PayRefundParams payRefundParams) {
        this.payParams = payParams;
        this.payRefundParams = payRefundParams;
    }
...
//            httpPost.addHeader("Authorization", "WECHATPAY2-SHA256-RSA2048 " + );
            ...
            rootNode.put("mchid", wxPayConfig.getMchId())
                    ...
                    .put("description", payParams.getDescription())
                    .put("notify_url", WxPayConfig.notifyUrl + "/" + wxPayConfig.getAppId())//回调地址
                    .put("out_trade_no", payParams.getOutTradeNo());
            rootNode.putObject("amount")
                    .put("total", payParams.getAmount().getTotal());//支付金额单位是：分
            rootNode.putObject("payer")
                    .put("openid", payParams.getPayer().getOpenId());//支付者的openId
    ...
    public String queryOrder() {
        ...
        try {
            ...
            // 通过WechatPayHttpClientBuilder构造的HttpClient，会自动的处理签名和验签
            CloseableHttpClient httpClient = builder.build();
            HttpGet httpGet = new HttpGet(QUERY_API_URL + payParams.getTradeNo() + "?mchid=" + wxPayConfig.getMchId());
...
//    }
    ...
    public Boolean refund() {
        try {
            ...
            ObjectNode rootNode = objectMapper.createObjectNode();
            rootNode.put("transaction_id", payRefundParams.getOutTradeNo())
                    .put("out_refund_no", payRefundParams.getOutRefundNo())
                    .put("reason", payRefundParams.getReason())
                    .put("notify_url", WxPayConfig.notifyUrl + "/" + wxPayConfig.getAppId());
            rootNode.putObject("amount")
                    .put("refund", payRefundParams.getAmount().getRefund())//金额单位是：分
                    .put("total", payRefundParams.getAmount().getTotal())//原订单金额
            ...
            if(statusCode == 200) return Boolean.TRUE;
            throw new BusinessException("退款失败", statusCode, "请查看错误码文档");
        } catch (JsonMappingException e) {
            throw new RuntimeException(e);
        } catch (NotFoundException e) {
            throw new RuntimeException(e);
    ...
    //构建HttpClient
    public WechatPayHttpClientBuilder getWechatPayHttpClientBuilder() throws NotFoundException {
        PrivateKey privateKey = loadPrivateKey(wxPayConfig.getMchId());
        //获取验签器
        Verifier verifier;
        try {
            verifier = getVerifier(wxPayConfig.getMchId());
        ...
        WechatPayHttpClientBuilder builder = WechatPayHttpClientBuilder.create()
                .withMerchant(wxPayConfig.getMchId(), wxPayConfig.getMerchantSerialNumber(), privateKey)
    ...
    public void getCertificate() throws URISyntaxException, IOException, NotFoundException {
     ...
     * 获取验签器
     **/
    public Verifier getVerifier(String merchantId) throws
        ...
        try {
            ...
            certificatesManager.putMerchant(merchantId, new WechatPay2Credentials(merchantId,
                    new PrivateKeySigner(wxPayConfig.getMerchantSerialNumber(), privateKey)), wxPayConfig.getApiV3Key().getBytes(StandardCharsets.UTF_8));
            verifier = certificatesManager.getVerifier(merchantId);
            return verifier;
        }
    }
...
/////////////////////////////////////////// 如果需要手动生成签名可使用以下方法///////////////////////////////////////////////////////
     ...
     */
    public static String getToken(String method, URL url, String body, WxPayConfig wxPayConfig) throws Exception {
        //下面有这个方法，生成随机数
        ...
        //下面有这个sign计算签名的方法
        String signature = sign(message.getBytes("utf-8"), wxPayConfig.getMchId());
...
//        第二步，获取请求的绝对URL，并去除域名部分得到参与签名的URL。如果请求中有查询参数，URL末尾应附加有'?'和对应的查询字符串。
     ...
     * 调起微信支付签名
     **/
    public String getAppletToken(String appid, String prepay_id, String nonceStr, Long timestamp) throws Exception {
